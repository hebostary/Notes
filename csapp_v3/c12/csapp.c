                 ...
                 const struct addrinfo *hints, struct addrinfo **res)
{
    ...
    /* getaddrinfo失败，不能使用perror或者stderror来生成错误消息，而是用gai_stderror将错误码转成错误消息  */
...
int open_listenfd(char *port) 
{
    ...
    /* Get a list of potential server addresses */
    memset(&hints, 0, sizeof(struct addrinfo));  /* hint结构中除了后面设置的字段，其余整数字段必须设置为为0，指针字段必须为空 */
    ...
    for (p = listp; p; p = p->ai_next) {
        /* Create a socket descriptor */
        /* Notes: socket返回的套接字仅是部分打开的，还不能用于读写, 如何完成打开套接字的工作，取决于是服务端还是客户端 */
    ...
    /* Notes: 默认情况下， 内核会认为socket函数创建的描述符对应于主动套接字（active socket），它存在于一个连接的客户端
    服务器调用listen函数告诉内核，描述符是被服务器使用而不是客户端使用 */
